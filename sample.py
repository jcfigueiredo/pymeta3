...
class Grammar(GrammarBase):
    ...
    def rule_grammar(self):
        ...
        self.considerError(lastError)
        return (_G_python_4, self.currentError)
class NullOptimizer(GrammarBase):
    def rule_opt(self):
        _locals = {'self': self}
        self.locals['opt'] = _locals
        def _G_or_1():
            def _G_listpattern_1():
                _G_exactly_1, lastError = self.exactly('Apply')
                self.considerError(lastError)
                _G_apply_2, lastError = self._apply(self.rule_anything, "anything", [])
                self.considerError(lastError)
                _locals['ruleName'] = _G_apply_2
                _G_apply_3, lastError = self._apply(self.rule_anything, "anything", [])
                self.considerError(lastError)
                _locals['codeName'] = _G_apply_3
                def _G_listpattern_4():
                    ...
                    _G_many_2, lastError = self.many(_G_many_1)
                    self.considerError(lastError)
                    _locals['exprs'] = _G_many_2
                    return (_locals['exprs'], self.currentError)
            ...
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.apply(ruleName, codeName, *exprs)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_2():
            def _G_listpattern_1():
                _G_exactly_1, lastError = self.exactly('Exactly')
                self.considerError(lastError)
                _G_apply_2, lastError = self._apply(self.rule_anything, "anything", [])
                self.considerError(lastError)
                _locals['expr'] = _G_apply_2
                return (_locals['expr'], self.currentError)
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.exactly(expr)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_3():
            def _G_listpattern_1():
                ...
                self.considerError(lastError)
                _G_apply_2, lastError = self._apply(self.rule_opt, "opt", [])
                self.considerError(lastError)
                _locals['expr'] = _G_apply_2
                return (_locals['expr'], self.currentError)
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.many(expr)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_4():
            def _G_listpattern_1():
            ...
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.many1(expr)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_5():
            def _G_listpattern_1():
            ...
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.optional(expr)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_6():
            def _G_listpattern_1():
                _G_exactly_1, lastError = self.exactly('Or')
                self.considerError(lastError)
                def _G_listpattern_2():
                    def _G_many_1():
                        _G_apply_1, lastError = self._apply(self.rule_opt, "opt", [])
                        self.considerError(lastError)
                        return (_G_apply_1, self.currentError)
                    _G_many_2, lastError = self.many(_G_many_1)
                    self.considerError(lastError)
            ...
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder._or(exprs)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_7():
            def _G_listpattern_1():
            ...
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.sequence(exprs)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_8():
            def _G_listpattern_1():
            ...
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder._not(expr)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_9():
            def _G_listpattern_1():
                _G_exactly_1, lastError = self.exactly('Lookahead')
                self.considerError(lastError)
                _G_apply_2, lastError = self._apply(self.rule_opt, "opt", [])
                self.considerError(lastError)
                _locals['expr'] = _G_apply_2
            ...
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.lookahead(expr)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_10():
            def _G_listpattern_1():
                ...
                _G_apply_3, lastError = self._apply(self.rule_opt, "opt", [])
                self.considerError(lastError)
                _locals['expr'] = _G_apply_3
                return (_locals['expr'], self.currentError)
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.bind(expr, name)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_11():
            def _G_listpattern_1():
                _G_exactly_1, lastError = self.exactly('Predicate')
                self.considerError(lastError)
                _G_apply_2, lastError = self._apply(self.rule_opt, "opt", [])
                self.considerError(lastError)
                _locals['expr'] = _G_apply_2
            ...
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.pred(expr)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_12():
            def _G_listpattern_1():
                _G_exactly_1, lastError = self.exactly('Action')
                self.considerError(lastError)
                _G_apply_2, lastError = self._apply(self.rule_anything, "anything", [])
                self.considerError(lastError)
                _locals['code'] = _G_apply_2
                return (_locals['code'], self.currentError)
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.action(code)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_13():
            def _G_listpattern_1():
                _G_exactly_1, lastError = self.exactly('Python')
                self.considerError(lastError)
                _G_apply_2, lastError = self._apply(self.rule_anything, "anything", [])
                self.considerError(lastError)
                _locals['code'] = _G_apply_2
            ...
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.expr(code)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
        def _G_or_14():
            def _G_listpattern_1():
                _G_exactly_1, lastError = self.exactly('List')
                self.considerError(lastError)
                _G_apply_2, lastError = self._apply(self.rule_opt, "opt", [])
                self.considerError(lastError)
                _locals['exprs'] = _G_apply_2
                return (_locals['exprs'], self.currentError)
            _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
            self.considerError(lastError)
            _G_python_3, lastError = eval('self.builder.listpattern(exprs)', self.globals, _locals), None
            self.considerError(lastError)
            return (_G_python_3, self.currentError)
    ...
    def rule_grammar(self):
        ...
        self.locals['grammar'] = _locals
        def _G_listpattern_1():
            _G_exactly_1, lastError = self.exactly('Grammar')
            self.considerError(lastError)
            _G_apply_2, lastError = self._apply(self.rule_anything, "anything", [])
            self.considerError(lastError)
            _locals['name'] = _G_apply_2
            def _G_listpattern_3():
                def _G_many_1():
                    _G_apply_1, lastError = self._apply(self.rule_rulePair, "rulePair", [])
                    self.considerError(lastError)
                    return (_G_apply_1, self.currentError)
                _G_many_2, lastError = self.many(_G_many_1)
                self.considerError(lastError)
        ...
        _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
        self.considerError(lastError)
        _G_python_3, lastError = eval('self.builder.makeGrammar(rs)', self.globals, _locals), None
        self.considerError(lastError)
        return (_G_python_3, self.currentError)
    ...
    def rule_rulePair(self):
        ...
        def _G_listpattern_1():
            _G_exactly_1, lastError = self.exactly('Rule')
            self.considerError(lastError)
            _G_apply_2, lastError = self._apply(self.rule_anything, "anything", [])
            self.considerError(lastError)
            _locals['name'] = _G_apply_2
            _G_apply_3, lastError = self._apply(self.rule_opt, "opt", [])
            self.considerError(lastError)
            _locals['rule'] = _G_apply_3
            return (_locals['rule'], self.currentError)
        _G_listpattern_2, lastError = self.listpattern(_G_listpattern_1)
        self.considerError(lastError)
        _G_python_3, lastError = eval('self.builder.rule(name, rule)', self.globals, _locals), None
        self.considerError(lastError)
        return (_G_python_3, self.currentError)
